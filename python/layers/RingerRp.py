...
class RingerRp(Layer):
  ...
  def call(self, input):
    ...
    #Ea_sum = K.reduce_sum( Ea, axis=0 )
    out = (Ea*rb)/ Ea_sum
    #out = (Ea*rb)
    #print('****************************************') 
    #K.print_tensor(input, message='input = ')
    #K.print_tensor(Ea, message='Ea = ')
    #K.print_tensor(rb, message='rb = ')
    #K.print_tensor(Ea_sum, message='Ea_sum = ')
    #K.print_tensor(out, message='out = ')
    #print('****************** FIM *****************') 
    return out
